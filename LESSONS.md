...
## Email-gated lead magnet (Phase B) uses its own standalone Apps Script, not the main pipeline

`app/components/EmailGate.tsx` is a separate, reusable client component (currently only placed on `/rental-documents/checklist-ontario`) that POSTs to its own Google Apps Script Web App, distinct from the main rental-inquiry pipeline in `RentalReadinessSection.tsx`. This was deliberate, not an oversight:

- **A single Apps Script project can only have one `doPost(e)` handler.** Adding a second, differently-shaped POST handler to the *existing* bound script (the one behind the main lead form) would mean editing/risking that script to route between two payload shapes - exactly the kind of shared-blast-radius change that was explicitly ruled out. Instead, this uses a brand-new, standalone Apps Script project (created via script.google.com directly, not "Extensions > Apps Script" from inside the spreadsheet, which would have bound it to the same project as the existing script). It references the shared "KEY TO GTA LEADS CRM - MASTER" spreadsheet by ID via `SpreadsheetApp.openById(...)` rather than by implicit binding, and only ever touches the "Email Subscribers" tab.
- Mirrors the exact `fetch(url, { method: "POST", mode: "no-cors", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) })` shape already used by the main pipeline, for the same reason it's used there: Apps Script Web Apps don't return CORS headers, so `no-cors` is required, and this makes the fetch promise resolve on any reachable response (even a script-side error) and only reject on genuine network failure (DNS/connection level) - the `catch` block is the *only* thing that will trigger the visible error state, tested by forcing the browser offline rather than relying on a bad URL (a bad-but-valid-looking URL still resolves "successfully" under `no-cors`, same as it would for the main form).
- `trackEvent()` from `app/lib/analytics.ts` is reused as-is (not forked) to fire `generate_lead_magnet` - see the earlier entry in this file for why that function must call `window.gtag()` and not push to `dataLayer` directly.
- **Apps Script deployments are versioned separately from the script's saved code.** Editing the script and saving it does *not* change what the live Web App URL actually runs - a new deployment version has to be published (Deploy → Manage deployments → edit → New version → Deploy) for code changes to take effect. Forgetting this step is the most common way this kind of integration silently keeps serving old behavior after a "fix."

## Never name a `public/` subfolder after the tail segment of an existing app route

The two lead-magnet PDFs were originally placed at `public/documents/*.pdf`. That 404'd in production (`next build` + `next start` - the same thing Vercel runs) even though `next dev` served them fine. Isolated it with a set of controlled tests, all on the same clean build: a brand-new root-level file worked, a brand-new subfolder named `stuff` worked, a subfolder named `docs` worked, a subfolder named `help-gta` (itself a substring of the route `newcomer-rental-help-gta`) worked - only `documents` failed, and `documents` happens to be the exact tail segment of the existing route folder `app/rental-documents/checklist-ontario`. Ruled out (each with a real test, not a guess): stale `.next` cache (full wipe + rebuild), the Sentry `withSentryConfig` wrapper (removed it, rebuilt, still 404'd), file permissions, and the HTTP client itself (reproduced with both `curl` and PowerShell's `Invoke-WebRequest`).

Never got a 100%-certain confirmation of the exact Turbopack mechanism (a same-name test rename of the actual route folder hit a Windows file lock, likely the IDE holding the file open, so that specific test was abandoned rather than forced), but the pattern - only the one name colliding with a real route's tail segment breaks, everything else doesn't - was consistent and reproducible enough to act on.

**Rule going forward:** before adding a new `public/` subfolder, check it doesn't match the last hyphenated segment of any existing `app/` route folder name. The PDFs now live at `public/pdfs/` instead - a name confirmed not to collide with anything in this project. If this bug resurfaces under a different name later, it's a Turbopack production-build asset-naming issue, not something wrong with the specific files or component.
